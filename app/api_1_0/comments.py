...
def get_post_comments(id):
    ...
    prev = None
    if pagination.has_prev:
        prev = url_for('api.get_post_comments', id=post.id, page=page-1, _external=True)  # 本书源码：'api.get_comments'
    next = None
    if pagination.has_next:
        next = url_for('api.get_post_comments', id=post.id, page=page+1, _external=True)  # 本书源码：'api.get_comments
    return jsonify({
        'comments': [comment.to_json() for comment in comments],
        'prev': prev,
        'next': next,
